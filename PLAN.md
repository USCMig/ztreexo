...
**The privacy mechanism this project spent Phases 6b and 6c on is not the one
the ecosystem will use** ([D44](docs/design.md)). Recording it here so the code
is read for what it is. `spendability-pir` hides the bucket index with
SimplePIR; we hide it in a crowd, and [D40](docs/design.md) measured that crowd
at 1 per wallet. Theirs is deployed and wired into wallets. The cohort and the
bridge service are kept, not deleted, for three reasons that are real and
small: they cover the full history rather than a 289-day window, they return a
proof rather than an answer, and they are the only non-membership primitive
here a *validator* could use. If a later phase needs to cut scope, this is the
first place to look — and the argument for cutting is written down above rather
